...
export default {
  async setUser(user) {
    ...
    const photosComp = document.querySelector('.components-user-photos');
    ...
    for (const photo of photos.items) {
      ...
      element.classList.add('.components-user-photo');
  ...
  handleEvents() {
    document.querySelector('.component-user-photos').addEventListener('click', async(e) => {
      if (e.target.classList.contains('.components-user-photo')) {
